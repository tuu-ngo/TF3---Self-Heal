...
```bash
# QUAN TRỌNG: cd vào thư mục repo trước (để chạy được scripts/... ở §4).
cd ~/Documents/capstone-huutai/TF3-Self-Heal-Agent-AWS   # đổi cho đúng đường dẫn máy bạn
...
**Service mục tiêu = `emailservice`** (ví dụ — không nằm trên đường checkout chính nên gãy tạm cũng ít ảnh hưởng UI). Các lệnh dưới dùng thẳng tên `emailservice`; **muốn service khác thì thay chữ `emailservice`** trong lệnh (KHÔNG dùng biến `$SVC` — biến không truyền sang terminal khác, dễ rỗng → lỗi `resource/name form`).
...
| **T2 — Pod watch** | `kubectl -n tenant-a get pods -l app=emailservice -w` | pod OB restart/heal thật (RESTARTS tăng, rồi Running lại) |
| **T3 — Rollout/replicaset** | `kubectl -n tenant-a rollout status deploy/emailservice` | trạng thái deployment sau RESTART |
...
```bash
kubectl -n tenant-a patch deploy emailservice --type=strategic -p '{"spec":{"template":{"spec":{"containers":[{"name":"server","command":["sh","-c","echo boom; exit 1"]}]}}}}'
...
```bash
kubectl -n tenant-a patch deploy emailservice --type=json -p '[{"op":"remove","path":"/spec/template/spec/containers/0/command"}]'
...
```bash
kubectl -n tenant-a set resources deploy/emailservice --limits=memory=12Mi
...
### Bước 3 — KHÔI PHỤC
```bash
kubectl -n tenant-a set resources deploy/emailservice --limits=memory=128Mi
kubectl -n tenant-a rollout status deploy/emailservice
```
...
**(Git Bash):**
```bash
kubectl -n self-heal-system exec -i deploy/ai-engine -- python - < scripts/demo_ai_smoke_ob.py
```
**(PowerShell):**
```powershell
...
```
Query hữu ích tại http://localhost:9090 (thay `emailservice` bằng service bạn demo nếu khác):
| Mục đích | PromQL |
|---|---|
...
kubectl -n tenant-a get events -w | grep -i "emailservice"
```
...
kubectl -n tenant-a patch deploy emailservice --type=json -p '[{"op":"remove","path":"/spec/template/spec/containers/0/command"}]' 2>/dev/null
# trả memory limit (Demo B)
kubectl -n tenant-a set resources deploy/emailservice --limits=memory=128Mi 2>/dev/null
kubectl -n tenant-a rollout status deploy/emailservice
kubectl -n tenant-a get pods -l app=emailservice          # phải Running 1/1
```
...
| `arguments in resource/name form must have a single resource and name` | `deploy/` bị thiếu tên (biến rỗng). Guide đã dùng tên thẳng `emailservice`; nếu tự dùng biến, nhớ `export SVC=...` trong **cùng** terminal. |
| `scripts/demo_ai_smoke_ob.py: No such file` | Bạn chưa `cd` vào thư mục repo (§1). Chạy `cd` tới thư mục dự án rồi thử lại. |
...
| OB service không xanh lại | Chạy §6 (gỡ command + trả memory). Nếu vẫn lỗi: `kubectl -n tenant-a rollout undo deploy/emailservice`. |
