"""Demo AI smoke test — detect -> decide -> verify trên AI engine LIVE.
Chạy BÊN TRONG pod ai-engine (đã có deps + tự gọi 127.0.0.1:8080), qua:
...
Không cần cài gì ở máy local; chỉ cần kubectl trỏ đúng cluster.
"""
import urllib.request, urllib.error, json, uuid, datetime, time

BASE = "http://127.0.0.1:8080"
TENANT = str(uuid.uuid4())  # tenant_id PHẢI là UUID v4 (engine validate)
...
pts = [{
    ...
    "labels": {"system": "K8S_NATIVE", "namespace": "tenant-a",
               "deployment": "cdo-sample-api", "container": "podinfo"},
} for i in range(180)]

st, b, dt = post("/v1/detect", {"telemetry_window": pts})
if not isinstance(b, dict):
    print(f"[DETECT] HTTP {st} in {dt}s -> ERROR: {b}")
    raise SystemExit(1)
...
if isinstance(b2, dict) and b2.get("action_plan"):
    ap = b2["action_plan"][0]
    print(f"[DECIDE] {st2} -> runbook={b2.get('matched_runbook')} "
          f"action={ap['action']} type={b2.get('pattern_type')}")
else:
...
def win(err):
    return [{"ts": now.isoformat().replace("+00:00", "Z"), "tenant_id": TENANT,
             "service": "cdo-sample-api", "signal_name": "service_error_rate", "value": err,
             "labels": {"system": "K8S_NATIVE", "namespace": "tenant-a", "deployment": "cdo-sample-api"}}
            for _ in range(60)]


act = {"action": "PATCH_MEMORY_LIMIT", "target": "deployment/cdo-sample-api",
